...
function setSessionCookie($username): void
{
    // Start a session
    session_start();
    // Save the session ID and last access time in a cookie
    $cookieValue = session_id() . '|' . time() . '|' . $username;
    setcookie('session_cookie', $cookieValue, time() + 60 * 60); // Cookie expires in 1 hour
}
